...
export interface PortalUpdatesProps {
  api: (path: string, opts?: Record<string, unknown>) => Promise<{ data?: { items?: UpdateItem[] } }>
  title?: string
  subtitle?: string
  emptyMessage?: string
}
...
function TypeBadge({ updateType }: { updateType: string | undefined }) {
...
function UpdateCard({ item, getSignedUrl }: { item: UpdateItem; getSignedUrl: (updateId: string, attId: string) => Promise<{ url?: string; data?: { url?: string } }> }) {
...
export function PortalUpdates({ api, title, subtitle, emptyMessage }: PortalUpdatesProps) {
  var [items, setItems] = useState<UpdateItem[] | null>(null)
