...
export interface UpdateAttachmentsProps {
  ...
  getSignedUrl?: ((updateId: string, attId: string) => Promise<{ url?: string; data?: { url?: string } }>) | null
  updateId?: string
  compact?: boolean
}

var DRIVE_ICON_COLORS: Record<string, string> = {
...
var FILE_ICONS: Record<string, { icon: string; color: string }> = {
...
function fileIconFor(filename: string | undefined): { icon: string; color: string } {
...
function ImageAttachment({ att, signedUrl, loading }: { att: Attachment; signedUrl: string | null; loading: boolean }) {
...
function DriveAttachment({ att }: { att: Attachment }) {
...
function LinkAttachment({ att }: { att: Attachment }) {
  var domain = ''
...
function R2Attachment({ att, getSignedUrl, updateId }: {
  att: Attachment
  getSignedUrl: ((updateId: string, attId: string) => Promise<{ url?: string; data?: { url?: string } }>) | null | undefined
  updateId: string | undefined
}) {
  var [signedUrl, setSignedUrl] = useState<string | null>(null)
  ...
  useEffect(function() {
    ...
    getSignedUrl(updateId || '', att.id).then(function(res) {
      setSignedUrl(res && res.data ? (res.data.url || null) : (res && res.url ? res.url : null))
      setLoading(false)
    }).catch(function() {
      setLoading(false)
    })
...
export function UpdateAttachments({ attachments, getSignedUrl, updateId, compact }: UpdateAttachmentsProps) {
  var parsed: Attachment[] | null = Array.isArray(attachments) ? attachments : null
