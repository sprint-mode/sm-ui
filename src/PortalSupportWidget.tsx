...
// COMMS-UNIFY-1d Phase 8. TypeScript version.
...
export function PortalSupportWidget(props: PortalSupportWidgetProps) {
  ...
  var _chatEnabled = useState<boolean | null>(null); var chatEnabled = _chatEnabled[0]; var setChatEnabled = _chatEnabled[1]
  var _messages = useState<ChatMsg[]>([]); var messages = _messages[0]; var setMessages = _messages[1]
  ...
  var _tickets = useState<Ticket[]>([]); var tickets = _tickets[0]; var setTickets = _tickets[1]
  ...
  function sendMessage() {
    ...
    fetch(url, {
      ...
      .then(function(d) {
        if (d.ok) {
          var aiMsg: ChatMsg = { role: 'assistant', content: d.message, id: 'a_' + Date.now() }
          if (d._debug) aiMsg._debug = d._debug
          setMessages(function(prev: ChatMsg[]) { return prev.concat([aiMsg]) })
        } else {
          setMessages(function(prev: ChatMsg[]) { return prev.concat([{ role: 'assistant', content: d.error || 'Sorry, something went wrong.', id: 'err_' + Date.now() }]) })
        }
      })
      .catch(function() {
        setMessages(function(prev: ChatMsg[]) { return prev.concat([{ role: 'assistant', content: 'Connection error. Please try again.', id: 'err_' + Date.now() }]) })
      })
  ...
  // Copy debug trace
  function copyDebug(debugData: Record<string, unknown>) {
    if (!debugData) return
      ...
      React.createElement('div', { style: s.body },
        ...
        tab === 'chat' && React.createElement('div', { style: { display:'flex', flexDirection:'column', height:'100%' } },
          React.createElement('div', { style: { flex:1, padding:'10px 12px', display:'flex', flexDirection:'column', gap:8, overflowY:'auto', minHeight:160 } },
            messages.map(function(msg: ChatMsg) {
              ...
              return React.createElement('div', { key: msg.id, style: { display:'flex', flexDirection:'column' } },
                ...
                isAi && msg._debug && isDebug && React.createElement('div', { style: { alignSelf:'flex-start', marginTop:2, display:'flex', gap:4 } },
                  React.createElement('button', {
                    style: s.traceBtn,
                    onClick: function() { if (msg._debug) copyDebug(msg._debug) },
        ...
        tab === 'form' && React.createElement('div', { style: { padding:14 } },
          ...
          React.createElement('input', {
            ...
            value: ticketSubject,
            onChange: function(e: any) { setTicketSubject(e.target.value) },
          }),
          ...
          React.createElement('textarea', {
            ...
            value: ticketBody,
            onChange: function(e: any) { setTicketBody(e.target.value) },
          }),
          React.createElement('button', {
