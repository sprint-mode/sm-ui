...
var ENDPOINTS: Record<'contact' | 'company', EndpointConfig> = {
...
type AvatarMode = null | 'picker' | 'saving'

export function AvatarUpload({ entityType, entityId, currentUrl, initials, source, apiBase, onUpdate, size }: AvatarUploadProps) {
  var base = apiBase || DEFAULT_API
  var sz = size || 56
  var cfg = ENDPOINTS[entityType] || ENDPOINTS.contact
  var [mode, setMode] = useState<AvatarMode>(null)
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      var data: { ok: boolean; data?: { photo_url?: string; logo_url?: string }; error?: string } = await res.json()
      if (data.ok && data.data) {
        var newUrl = data.data.photo_url || data.data.logo_url || null
  ...
  async function handleRemove() {
    ...
    try {
      ...
      var data: { ok: boolean; error?: string } = await res.json()
      if (data.ok) {
  ...
  var circle = (
    <div style={{
      ...
      {currentUrl
        ? <img src={currentUrl} alt="" style={{ width: sz, height: sz, objectFit: 'cover', display: 'block' }} onError={function(e) { (e.target as HTMLImageElement).style.display = 'none' }} />
        : (initials || '?')
      }
    </div>
  )
